...
@RestController
@RequestMapping("/api/crew/events")
...
public class CrewEventController {
	...
	public List<EventResponse> list(Authentication authentication) {
		return eventService.listForCrew(currentUserService.require(authentication));
	}

	@PostMapping("/{eventId}/applications")
	@ResponseStatus(HttpStatus.CREATED)
	public EventResponse apply(Authentication authentication, @PathVariable Long eventId) {
		return eventService.applyAsCrew(currentUserService.require(authentication), eventId);
	}
}
